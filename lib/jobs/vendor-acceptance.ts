 ...
 *
 * Escalation state is stored on the sub-order (acceptanceEscalation) so
...
export async function runVendorAcceptanceJob(): Promise<RunAcceptanceJobResult> {
  ...
  const timeoutMinutes =
    rules.deliveryTiming?.vendorAcceptanceTimeoutMinutes ?? 30;
  const now = Date.now();
  ...
  for (const order of orders) {
    ...
    for (let i = 0; i < order.subOrders.length; i++) {
      ...
      const vendor =
        vendorDoc && typeof vendorDoc === 'object' && vendorDoc.vendorSettings
          ? vendorDoc
          : await Organization.findById(sub.vendorId);
      const phone: string | undefined = vendor?.vendorSettings?.contactPhone;
    ...
    if (mutated) {
      const allCancelled = order.subOrders.every(

...
/**
 * True when the order originates from a contract flow. Detected via either a
 * `contractId` reference (forward-compat with an extracted Contract model) or
 * the legacy `contractDurationMonths` field currently persisted on Order.
 * Contract-like orders auto-accept for the vendor by default (issue #6).
 */
 ...
 * auto-accept when vendor settings opt in (vendorSettings.autoAcceptOrders).
 */
...
/**
 * Recompute the parent order status from its sub-order statuses. Mirrors the
 * mock-data helper so real + mock stay in sync.
 */
export function recomputeOrderStatus(
  order: IOrder | { subOrders: ISubOrder[]; status: IOrder['status'] }
): void {
 ...
 */
export function allActiveSubOrdersAccepted(
  subs: Pick<ISubOrder, 'status'>[]
): boolean {

...
When every non-cancelled sub-order is `ACCEPTED` the PaymentIntent is captured
and per-vendor Stripe Connect transfers fire (`captureAndTransferForOrder`).
The parent Order transitions to `CONFIRMED`.
...
- `proceed` — captures the PaymentIntent, transfers ACCEPTED sub-order totals
  to their vendors, then issues Stripe partial refunds for every CANCELLED
  sub-order total. This preserves the existing manual-capture Connect flow
  instead of adjusting capture amounts.
...
backward compatibility with existing schedulers, so both entry points share
the same phone-number source (`vendorSettings.contactPhone`) — fixing the
prior use of the top-level `phoneNumber` field.

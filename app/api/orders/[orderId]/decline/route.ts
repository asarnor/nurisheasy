 ...
 * released (uncaptured). Otherwise the response instructs the consumer to hit
 * `/resolve-partial` to pick between proceed vs cancel_all.
 */
    ...
    if (await shouldUseMockData(request)) {
      ...
      return NextResponse.json({
        ...
        nextAction: requiresConsumerChoice
          ? {
              endpoint: `/api/orders/${result._id}/resolve-partial`,

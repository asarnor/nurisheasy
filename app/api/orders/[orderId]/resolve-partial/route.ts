 ...
 *     total of each cancelled sub-order after capture. This is pragmatic: we
 *     avoid changing capture amounts (which Stripe supports but complicates
 *     Connect/transfer math) and instead let Stripe issue partial refunds for
 *     the cancelled vendor totals.
 */
  ...
  try {
    ...
    if (await shouldUseMockData(request)) {
      ...
      return NextResponse.json({
        order: result,
        message:
          action === 'cancel_all' ? 'Order cancelled' : 'Order proceeding with accepted sub-orders',
      });
    }
    ...
    // Consumer scoping — only the ordering consumer (or admin) may resolve.
    if (
    ...
    if (action === 'cancel_all') {
      ...
      await order.save();
      return NextResponse.json({ order: order.toObject(), message: 'Order cancelled' });
    }
    ...
    if (stillPending) {
      return NextResponse.json(
        { error: 'Cannot proceed while some sub-orders remain pending vendor acceptance' },
        { status: 400 }
      );
    }

    const cancelledTotals = order.subOrders
      .map((so, idx) => ({ subOrderIndex: idx, total: so.vendorTotal, status: so.status }))
      .filter((entry) => entry.status === 'CANCELLED')
      .map(({ subOrderIndex, total }) => ({ subOrderIndex, total }));

    const hasAccepted = order.subOrders.some((so) => so.status === 'ACCEPTED');
    if (!hasAccepted) {
      // Nothing to capture — cancel the whole thing.
      await cancelPaymentIntentSafe(order.paymentIntentId);
      recomputeOrderStatus(order);
      await order.save();
      return NextResponse.json({ order: order.toObject(), message: 'No accepted sub-orders — order cancelled' });
    }
    ...
    try {
      ...
      return NextResponse.json(
        { error: 'Payment capture failed', details: (error as Error).message },
        { status: 500 }
      );
    }

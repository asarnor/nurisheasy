...
export default function KitchenDisplaySystemPage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (typeof data?.platformRules?.vendorAcceptanceTimeoutMinutes === 'number') {
          setAcceptanceTimeoutMinutes(data.platformRules.vendorAcceptanceTimeoutMinutes);
        }
        ...
        // Ring the KDS bell for newly-seen PENDING orders (skip on very first
        // poll to avoid a stampede of beeps when the page loads).
  ...
  const handleDecline = async (orderId: string) => {
    ...
    const index = parseInt(reasonLabel, 10) - 1;
